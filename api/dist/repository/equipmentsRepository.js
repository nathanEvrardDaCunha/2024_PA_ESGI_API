"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.updateEquipment = exports.createEquipment = exports.getEquipmentById = exports.getAllEquipment = void 0;
const index_1 = require("../index");
function getAllEquipment() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return yield index_1.prisma.equipment.findMany();
        }
        catch (error) {
            console.error('Error fetching equipments:', error);
            throw error;
        }
    });
}
exports.getAllEquipment = getAllEquipment;
function getEquipmentById(id) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return yield index_1.prisma.equipment.findUnique({ where: { id } });
        }
        catch (error) {
            console.error('Error fetching equipment by ID:', error);
            throw error;
        }
    });
}
exports.getEquipmentById = getEquipmentById;
function createEquipment(data) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return yield index_1.prisma.equipment.create({ data });
        }
        catch (error) {
            console.error('Error creating equipment:', error);
            throw error;
        }
    });
}
exports.createEquipment = createEquipment;
function updateEquipment(id, data) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            return yield index_1.prisma.equipment.update({
                where: { id },
                data,
            });
        }
        catch (error) {
            console.error('Error updating equipment:', error);
            throw error;
        }
    });
}
exports.updateEquipment = updateEquipment;
